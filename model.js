...
function SurfaceGrid(nu, nv) {
    ...
    this.generate = function() {
        ...
        for (let iv = 0; iv <= this.nv; iv++) {
            ...
            for (let iu = 0; iu <= this.nu; iu++) {
                ...
                const du = twoPi / this.nu;
                const dv = twoPi / this.nv;
                const pu = klein(u + du, v);
                const pv = klein(u, v + dv);
                const dPdu = vec3Sub(pu, p);
                const dPdv = vec3Sub(pv, p);

                const nParam = vec3Normalize(vec3Cross(dPdu, dPdv));
                let tParam = vec3Normalize(dPdu);
                const sign = (vec3Dot(vec3Cross(nParam, tParam), vec3Normalize(dPdv)) < 0.0) ? -1.0 : 1.0;
                tangents.push(tParam[0], tParam[1], tParam[2], sign);
            }
        }
    ...
    this.drawTriangles = function() {
        if (!this.positionBuffer || !this.normalBuffer || !this.texCoordBuffer || !this.tangentBuffer || !this.indexBuffer || this.indexCount === 0) {
            return;
        }
